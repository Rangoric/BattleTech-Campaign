...
export const applySearchToGroup = (group: IGroup, searchTerm: string) => {
  return (
    ...
    group.territorialHoldings?.some((holding) =>
      applySearchToTerritory(holding, searchTerm)
    ) ||
    group.corporateHoldings?.some((holding) =>
      applySearchToCorporateHolding(holding, searchTerm)
    )
  );
};
...
export const applySearchToTerritory = (
  holding: ITerritorialHolding,
  searchTerm: string
) => {
  return (
...
export const applySearchToCorporateHolding = (
  holding: ICorporateHoldings,
  searchTerm: string
) => {
  return (
    holding.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (holding.description &&
      holding.description
        ...
        .toLowerCase()
        .includes(searchTerm.toLowerCase())) ||
    holding.holdings?.some((item) =>
      applySearchToCorporateItem(item, searchTerm)
    )
  );
};

export const applySearchToCorporateItem = (
  item: ICorporateHolding,
  searchTerm: string
) => {
  return (
...
export const applySearchToEvent = (event: IEvent, searchTerm: string) => {
  return (
